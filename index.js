...
const thinClientRoutes = require('./routes/thinClient');
...
app.use('/runtime/thin-client', requireApiKey, thinClientRoutes);
app.get('/healthz', (_, res) => res.json({ ok: true, service: 'botify-x-core', ts: Date.now() }));
...
registry.init().then(async () => {
  if (process.env.DATABASE_URL) {
    try { await hydrateFromPg(); }
    catch (e) { console.error('[BOTIFY X] Hydration failed (non-fatal):', e.message); }
  }

  const validSessions = await registry.getAllValid();
  if (validSessions.length) {
    console.log('[BOTIFY X] Restoring ' + validSessions.length + ' session(s)...');
...
function shutdown(sig) {
  console.log('[BOTIFY X] ' + sig + ' — shutting down...');
